...
def Delete(L, pages):
    ...
    #   Precondition:  @pages is an int >= 0
    assert type(pages) is int, "pages is not an int"
    assert pages >= 0, "pages is not >= 0"
    #   Precondition:  The length of @L >= @pages
    assert len(L)>=pages, "The length of the list 'L' is less then the pages being removed. The length of L must be >= pages"
    ##############################################################################
    ...
    ######################## POSTCONDTION CHECKS #################################
    #   Postcondition: The final @pages of @L are removed from @L
    assert(origListLen - len(L) == pages), "The data in L was incorrectly removed."
    ##############################################################################
...
def Split(pdf_filename, names_filename, perFile, skipPages):
    ...
    ######################## PRECONDTION CHECKS #################################
    #   Precondition:   @pdf_filename is a string
    assert type(pdf_filename) is str, "pdf_filename is not str"
    #   Precondition:   The pdf file specified as @pdf_filename exists and is readable
    assert os.path.exists(pdf_filename), "pdf_filename does not exist in directory"
    #   Precondition:   The file defined by @pdf_filename must be a PDF
    assert os.path.splittext(os.path.normpath(pdf_filename))[1] == ".pdf", "pdf_filename is not a PDF"
    #   Precondition:   @names_filename is a string
    assert type(names_filename) is str, "names_filename is not str"
    #   Precondition:   If inputted, @names_filename must exist and be readable or is an empty string
    assert os.path.exists(names_filename) or len(names_filename) == 0, "names_filename does not exist in directory or is not blank"
    #   Precondition:   @perFile is an int > 0
    assert type(perFile) is int, "perFile is not int"
    assert perFile > 0, "perFile is less than or equal to 0, must be greater than 0"
    #   Precondition:   @skipPages is an int >= 0 
    assert type(skipPages) is int, "skipPages is not int"
    assert skipPages >= 0, "skipPages is less than 0, must be greater than or equal to 0"
    #############################################################################
    ...
    ######################## PRECONDTION CHECKS #################################
    #   Precondition:   The number of pages in @pdf_filename must be > 0
    assert input_pdf.numPages > 0, "Number of pages in pdf is 0, must have at least 1 page"
    #   Precondition:   The subdirectory "allpages" must exist
    assert os.path.isdir(directory), "allpages subdirectory does not exist"
    #   Precondition:   @perFile must be <= total amount of pages in @pdf_filename
    assert perFile <= input_pdf.numPages, "The amount of pages per file is greater than the amount of pages in the input pdf"
    #############################################################################
    ...
    if len(names_filename) > 0:
        ...
        ######################## PRECONDTION CHECKS #################################
        #   Precondition:   @skipPages must be <= the amount of pages in @pdf_filename
        assert ((input_pdf.numPages) - skipPages // perFile) <= len(names), "skipPages should be less than or equal to total amount of pages in input PDF"
        #############################################################################
    ...
    #   Postcondition:  The amount of pdfs created will be equal to ((# of pages in @pdf_filename)
    #                   - @skipPages) // @perFile
    assert sum([len(file_names) for r, d, file_names in os.walk(directory)]) == ([input_pdf.numPages - skipPages] // perFile), "The function failed to create the correct number of files"
    ...
    for num in range(len(file_names)):
        ...
        #   Postcondition:  Created pdf files will either be named on the contents of @name_filename or as "PDFPage" 
        #                   followed by a number starting at 0
        assert os.path.exists(file_names[num]), "New PDF file " + file_names[num] + " does not exist"
        ...
        if num < len(file_names) - 1:
             assert output_pdf.numPages == perFile, "New PDF file " + file_names[num] + " does not have " + str(perFile) + " pages"
        else:
            assert output_pdf.numPages == (((inputLength) - skipPages) % perFile), "New PDF file " + file_names[num] + " does not have " + str((((input_pdf.numPages) - skipPages) % perFile)) + " pages"
...
def Merge(files_dir, pages):
    ...
    ######################## PRECONDTION CHECKS #################################
    #   Precondition:   @files_dir is a string
    assert type(files_dir) is str, "files_dir is not str"
    #   Precondition:   Directory defined in @files_dir must exist
    assert os.path.isdir(files_dir), "directory outlined in files_dir does not exist"
    #   Precondition:   @pages is an int >= 0
    assert type(pages) is int, "pages is not int"
    assert pages >= 0, "pages is less than 0, must be greater than or equal to 0"
    #############################################################################
    ...
    ######################## PRECONDTION CHECKS #################################
    #   Precondition:   At least 2 pdf files must exist in directory
    assert len(pdf_files) >= 2, "There is less than 2 PDF files in directory"
    #############################################################################
    ...
    #   Postcondition:  The number of pdf files created is equal to (# of PDF files in @files_dir)
    #                   // @pages
    assert origPDFCount // pages == count, "Failed to create correct number of PDF files"
    for num in range(count):
         #   Postcondition:  The new files will be named as "merged_file" followed by a number starting at 0
        assert mergedFiles[num] == "merged_file"+str(num)+".pdf", mergedFiles[num] + " is not a correct file name"
        #   Postcondition:  The number of pages in each file will equal to @pages
        assert PdfFileReader(os.path.join(files_dir, "merged_file"+str(num))+".pdf").numPages == pages, "The number of pages in each file does not equal pages, need to be equal to pages"
    #   Postcondition:  If a file with the same name as a created pdf file already exists in the directory,
    #                   the file will be overwritten with the new pdf file
    #   Unable to test this Postcondition
    ##############################################################################